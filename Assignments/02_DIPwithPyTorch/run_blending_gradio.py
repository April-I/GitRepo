...
def create_mask_from_points(points, img_h, img_w):
    ...
    #     f.write(np.array2string(mask, threshold=np.inf))
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    laplacian_foreground = torch.nn.functional.conv2d(foreground_img, laplacian_kernel, groups=3,padding='same')
    laplacian_blended = torch.nn.functional.conv2d(blended_img, laplacian_kernel, groups=3,padding='same')
    
    masked_foreground = torch.masked_select(laplacian_foreground, foreground_mask.bool())
    masked_blended = torch.masked_select(laplacian_blended, background_mask.bool())

    loss = torch.nn.functional.mse_loss(masked_foreground, masked_blended)
